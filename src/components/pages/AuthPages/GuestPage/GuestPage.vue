    ...
    <Card :showBack="true" backUrl="/">
      <div class="container">
        ...
        <div class="guest-form" v-if="!guestCreated">
          ...
          <!-- <div v-if="guestCreated"> -->
          <!-- Let the user book an appointment -->
          <!-- </div> -->
        </div>
        <div v-if="guestCreated" class="create-form">
          <CreateAppointmentForm :specificCounsellor="counsellor" :user="user" />
        </div>
      </div>
    </Card>
  </div>
...
export default {
  ...
  methods: {
    ...
    async registerGuest() {
      ...
      try {
        await UserService.registerUser(guest, Role.Guest);
