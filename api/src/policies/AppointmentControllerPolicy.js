...
async function updateAppointment(req, res, next) {
  ...
  switch (req.user.role) {
    ...
    case Roles.CLIENT:
      ...
      break;
    case Roles.COUNSELLOR: // If the users role if Counsellor the switch will cascade to admin, as they (currently) have the same update rights.
    case Roles.ADMIN:
  ...
  if (requestedAppointmentProperties.includes("startTime")) {
    ...
    if (startTime.isBefore(now)) return response.failure("You cannot reschedule an appointment to a past date.", 400);
  }
