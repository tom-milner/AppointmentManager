...
async function createAppointment(req, res) {
  ...
  try {
    ...
    mailer
      .confirmAppointment(createdAppointments, createdAppointments[0].clients, createdAppointments[0].counsellorId)
      .send();
  ...
  try {
    ...
    if (Object.keys(newAppointmentProperties).includes("startTime")) {
      ...
      if (requestedStartTime.isBetween(now, next24Hours))
        return response.failure("You cannot reschedule an appointment for the next day.", 400);
